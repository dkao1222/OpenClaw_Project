...
- QA 必須包含但不限於按鍵位置、按鍵尺寸、可點擊狀態、安全區內佈局、HUD 不重疊、核心遊戲物件、分數系統、暫停系統、失敗/重試流程與 runtime exception scan。
- Three-cycle QA：QA 不可只看前 10 秒影片；必須至少執行 3 complete playthrough cycles，每輪包含 menu、gameplay、game-over、retry/after-retry 證據，錄影長度依單局時長乘以三輪決定。
- Unity Test Framework 必須提供 EditMode tests 驗證 runtime probe schema 與核心狀態，PlayMode tests 驗證 scene load、HUD/probe presence、score progression 與 retry state hook。
...
- QA must include, but is not limited to, button position, button size, clickability, safe-area layout, non-overlapping HUD, core gameplay objects, scoring system, pause system, failure/retry flow, and runtime exception scan.
- Three-cycle QA: QA cannot pass from a first-10-second clip only; it must run at least 3 complete playthrough cycles, each with menu, gameplay, game-over, and retry/after-retry evidence, with recording length derived from single-run duration times three.
- Unity Test Framework must provide EditMode tests for runtime probe schema/core state and PlayMode tests for scene load, HUD/probe presence, score progression, and retry-state hooks.
