...
- 若視覺或音效素材來自 OpenGameArt、圖庫、音效庫或第三方來源，必須列出 exact license、attribution、store decision 與替代方案。
...
- If visual or audio assets come from OpenGameArt, an image library, a sound library, or another third-party source, list exact license, attribution, store decision, and fallback.
